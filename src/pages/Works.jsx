...
function Carousel({ slides }) {
  ...
  const getSlideStyles = (index) => {
    ...
    if (index === prevIndex)
      return "z-10 scale-90 opacity-60 -translate-x-8 md:-translate-x-48 -rotate-3 md:-rotate-6 bg-main";
    if (index === nextIndex)
      return "z-10 scale-90 opacity-60 translate-x-8 md:translate-x-48 rotate-3 md:rotate-6 bg-main";
  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={prev}
          className="absolute z-50 left-2 text-4xl font-bold md:left-20 md:text-6xl hover:scale-110 transition bg-white/50 rounded-full p-1 md:bg-transparent"
        >
          ‹
        </button>
        <button
          onClick={next}
          className="absolute right-2 font-bold md:right-20 z-50 text-4xl md:text-6xl hover:scale-110 transition bg-white/50 rounded-full p-1 md:bg-transparent"
        >
          ›
        </button>
          ...
          {slides.map((slide, i) => (
            ...
            >
              <div className="w-full aspect-video bg-black rounded-lg overflow-hidden relative mb-4 shadow-inner">
                ...
                {/* CLICK OVERLAY — only for IMAGES */}
                {slide.type !== "video" && (
                  <div
                    className="absolute inset-0 z-20 cursor-pointer"
                    onClick={() => {
                      if (i === current) setSelectedSlide(slide);
                    }}
                  />
                )}
              </div>
...
const slides = [
  ...
  {
    title: "KREATOR - Seven Serpents (OFFICIAL MUSIC VIDEO)",
    desc: "descrição da imagem",
    type: "video",
    src: "xPRvLYHKD-I"
  },
  {
    title: "Kreator director cut",
    desc: "descrição da imagem",
    type: "video",
    src: "IVhN9_BbpKM"
  },
